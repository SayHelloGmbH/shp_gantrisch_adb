...
class ParksView
{
			...
			if (! empty($accessibilities)) {
				$template_data['FILTER_ACCESSIBILITIES'] = '
					<div class="form_element mega_dropdown filter_accessibilities">
						<div class="form_group">
							<h4 aria-haspopup="true" role="button" tabindex="0" title="' . $this->api->lang->get('general_select_field') . ': ' . $this->api->lang->get('offer_accessibility') . '">
								<i class="deselect_icon" aria-hidden="true">m</i>
								<span class="selected_option"></span>
				...
				foreach ($accessibilities as $accessibility_id => $accessibility) {
					...
					$template_data['FILTER_ACCESSIBILITIES'] .= '
										<label role="button" aria-level="2" tabindex="0">
	...
	public function map($offers, $poi_detail = false, $offer_id = false)
	{
			...
			foreach ($offers as $offer) {
				...
				if (isset($offer->latitude) && ($offer->latitude > 0) && isset($offer->longitude) && ($offer->longitude > 0)) {
					...
					if ($offer->root_category == CATEGORY_EVENT) {
						...
						$date = parks_show_date(['date_from' => parks_mysql2form($date_from), 'date_to' => parks_mysql2form($date_to)], $this->api->lang);
					}
		...
		// Map option: Do not group offer layers in legend
		$config_ui['offerLayersNoGrouping'] = false;
		if (! empty($this->api->map_options['do_not_group_categories_in_legend']) && ($this->api->map_options['do_not_group_categories_in_legend'] == true)) {
			$config_ui['offerLayersNoGrouping'] = true;
		}
